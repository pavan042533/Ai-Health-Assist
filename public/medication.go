...
func (a medicationHandler) HandleScanPrescription(c *fiber.Ctx) error {
	// 1. Parse image
...
func (a medicationHandler) HandleReviewMedications(c *fiber.Ctx) error {
	var req struct {
		PatientID   uint       `json:"patient_id"`
		Medications []medication.MedInput `json:"medications"`
	}
	...
	// STEP 1: normalize drug names to RxNorm-style IDs
	mappedMeds := a.medicationService.MapMedications(req.Medications, config.DB)

	// // STEP 2: duplicate check
	// duplicates := a.medicationService.CheckDuplicates(req.PatientID, mappedMeds)

	// // STEP 3: interaction check
	// interactions := a.medicationService.CheckInteractions(req.PatientID, mappedMeds)

	// // STEP 4: dosage safety
	// dosageWarnings := a.medicationService.CheckDosage(req.PatientID, mappedMeds)

	// STEP 5: return validation result (NOT SAVING)
...
func HandleSaveMedications(c *fiber.Ctx) error {
	return c.SendString("HandleSaveMedications not implemented yet")
}

func newMedicationHandler(medService medication.MedicationService) *medicationHandler {
